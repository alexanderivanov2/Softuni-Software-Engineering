...
def get_player_choice_func(is_test=False):
    ...
    choices_for_test = [1, 2, 2, 3, 3, 4, 1, 5]  # "horizontal_win"
    # choices_for_test = [1, 2, 1, 2, 1, 2, 1]  # "vertical_win"
    # choices_for_test = [1, 2, 2, 4, 3, 3, 3, 7, 4, 4, 4]  # "right_diagonal_win"
    # choices_for_test = [4, 3, 3, 1, 2, 2, 2, 7, 1, 1, 1]  # "left_diagonal_win"
...
def has_win_condition(board, player, row_index, columns_index, win_condition_count=DEFAULT_WIN_CONDITION_COUNT):
    rows_count = len(board)
    columns_count = len(board[0])

    left_horizontal_values = [
        is_win_condition_value(board, row_index, columns_index + d, player, rows_count, columns_count)
        for d in
        range(win_condition_count)]
    right_horizontal_values = [
        is_win_condition_value(board, row_index, columns_index - d, player, rows_count, columns_count)
        for d in
        range(win_condition_count)]
    down_vertical_values = [
        is_win_condition_value(board, row_index + d, columns_index, player, rows_count, columns_count)
        for d in
        range(win_condition_count)]
    up_vertical_values = [is_win_condition_value(board, row_index - d, columns_index, player, rows_count, columns_count)
                          for d in
                          range(win_condition_count)]
    down_right_diagonal_values = [
        is_win_condition_value(board, row_index + d, columns_index + d, player, rows_count, columns_count) for d in
        range(win_condition_count)]
...
def print_board(board):
    print(f" {'  '.join([str(x) for x in range(1, 8)])}")
    for row in board:
        print(row)
