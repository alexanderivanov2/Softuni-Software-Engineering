...
def get_sequence_length(board, row_index, column_index, row_delta, column_delta, max_possible_length,
                        value):
    left_most_row_index = row_index
    right_most_row_index = row_index
    for i in range(max_possible_length):
        ...
        if in_range(current_row_index, len(board)) \
            ...
            left_most_row_index = current_row_index
        else:
            break
    for i in range(max_possible_length):
        current_row_index = row_index + row_delta * i
        ...
        if in_range(current_row_index, len(board)) \
                and in_range(current_column_index, len(board[0])) \
                and board[current_row_index][current_column_index] == value:
            right_most_row_index = current_row_index
        else:
            break
    return abs(right_most_row_index - left_most_row_index + 1)
...
def has_left_diagonal_win_condition(board, player, row_index, column_index, win_condition_count):
    return win_condition_count <= get_sequence_length(board, row_index, column_index, 1, 1, win_condition_count,
                                                      player)


def has_right_diagonal_win_condition(board, player, row_index, column_index, win_condition_count):
...
def has_win_condition(board, player, row_index, column_index, win_condition_count=DEFAULT_WIN_CONDITION_COUNT):
    return any([
        ...
        has_left_diagonal_win_condition(board, player, row_index, column_index, win_condition_count),
        has_right_diagonal_win_condition(board, player, row_index, column_index, win_condition_count),
    ])
...
def print_board(board):
    print(f" {'  '.join([str(el) for el in range(1,8)])}")
    for row in board:
        print(row)
...
def play(board, player=1):
    ...
    if has_win_condition(board, player, row_index, column_index):
        ...
        play(board, 2 if player == 1 else 1)
