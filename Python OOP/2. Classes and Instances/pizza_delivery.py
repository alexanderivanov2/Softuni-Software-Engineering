class PizzaDelivery:
    ...
    def make_order(self):
        self.ordered = True
        line_ingredients = ", ".join(f"{key}: {value}" for key, value in self.ingredients.items())
        return f"You've ordered pizza {self.name} prepared with {line_ingredients} and the price will be {self.price}lv."
