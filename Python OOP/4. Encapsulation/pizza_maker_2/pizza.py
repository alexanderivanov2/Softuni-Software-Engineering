class Pizza:
    ...
    def add_topping(self, topping):
        if len(self.toppings) == self.__toppings_capacity:
            ...
            self.toppings[topping.topping_type] *= 2
        else:
            self.toppings[topping.topping_type] = topping.weight

    def calculate_total_weight(self):
        total_weight = self.__dough.weight
        total_weight += sum([el for el in self.toppings.values()])
        return total_weight

#
# from project_pizza_maker_2.dough import Dough
# from project_pizza_maker_2.topping import Topping
#
#
